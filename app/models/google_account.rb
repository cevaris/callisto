...
class GoogleAccount < ActiveRecord::Base
  ...
  def owned_by?( user )
	...
	def list_files( folder_id=nil, title=nil, max_results=100, mime_type=nil, trashed=false )
		@client ||= api_client()
	    ...
	    # Rails.logger.info("Google Drive List Parameters: #{parameters}")
	    ...
	    if api_result.status == 200
	      	...
	      	children.items.each do |child|	
		        ...
		        begin
			        ...
			        if child.parents[0]['id'] == folder_id or folder_id == 'root' or folder_id.nil?
			        	result << child
			        end
		    	rescue
